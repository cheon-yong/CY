...
UCYItemInstance* UCYEquipmentComponent::EquipItem(TSubclassOf<UCYItemDefinition> InItemDefinition)
{
	...
	{
		Result = EquipmentList.AddEntry(InItemDefinition);
		if (Result != nullptr)
		{
			Result->OnEquipped();
...
bool UCYEquipmentComponent::ReplicateSubobjects(UActorChannel* Channel, FOutBunch* Bunch, FReplicationFlags* RepFlags)
{
	...
	for (FCYAppliedEquipmentEntry& Entry : EquipmentList.Entries)
	{
...
void UCYEquipmentComponent::UninitializeComponent()
{
	...
	for (const FCYAppliedEquipmentEntry& Entry : EquipmentList.Entries)
	{
...
TArray<UCYItemInstance*> UCYEquipmentComponent::GetEquipmentInstancesOfType(TSubclassOf<UCYItemInstance> InstanceType) const
{
