...
#include "CYTeamSubsystem.generated.h"

class AActor;
class ACYPlayerState;
class ACYTeamInfoBase;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnCYTeamDisplayAssetChangedDelegate, const UCYTeamDisplayAsset*, DisplayAsset);

USTRUCT()
struct FCYTeamTrackingInfo
{
	GENERATED_BODY()

public:
	UPROPERTY()
	TObjectPtr<ACYTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<ACYTeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<UCYTeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnCYTeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(ACYTeamInfoBase* Info);
	void RemoveTeamInfo(ACYTeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class ECYTeamComparison : uint8
{
...
UCLASS()
class CY_API UCYTeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	bool RegisterTeamInfo(ACYTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	bool UnregisterTeamInfo(ACYTeamInfoBase* TeamInfo);
	...
	const ACYPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	ECYTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	ECYTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UCYTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UCYTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	void NotifyTeamDisplayAssetModified(UCYTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnCYTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FCYTeamTrackingInfo> TeamMap;
