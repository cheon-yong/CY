...
class ICYTeamAgentInterface;
class UCYTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UCYTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements ICYTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UCYTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UCYTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<ICYTeamAgentInterface> TeamInterfacePtr;
